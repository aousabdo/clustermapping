...
pull_storm_gis_advisories <- function(storm_data_obj = NULL){
  # Function to get a list of gis storm advisories from the National Hurricane Center
...
#========================================================================================#
#============================ download_storm_gis_advisories =============================#
#========================================================================================#
download_storm_gis_advisories <- function(gis_advisories_vec = NULL
                                          , storm_key = NULL
                                          , storm_data_obj = NULL
                                          , advisory_num = NULL
                                          , download_all_advisories = FALSE){
  # Function to download gis storm advisories from the National Hurricane Center
  
  # fuction arguments
  # gis_advisories_vec: a vector containing a list of the gis advisories available for
  # a given storm. This is the output of the pull_storm_gis_advisories function
  # storm_key: storm key from NHC. If NULL then the storm_data_obj must be given
  # storm_data_obj: If gis_advisories_vec is not given then we can call the pull_storm_gis_advisories function 
  # on the storm_data_obj object to get a list of the gis storm advisories
  ...
  if(is.null(gis_advisories_vec)){
    if(is.null(storm_data_obj)){
      stop("\tI need either a gis_advisories_vec or a valid storm_data_obj. Quitting...\n")
    }else{
      # sine the gis_advisories_vec is null we have to get one by calling the pull_storm_gis_advisories function
      gis_advisories_vec <- pull_storm_gis_advisories(storm_data_obj = storm_data_obj)
      
      # get the key for the storm
      key <- storm_data_obj[["fstadv"]] %>% pull(Key) %>% first()
    }
  }else if(is.null(storm_key)){
    # get the key for the storm
    key <- storm_data_obj[["fstadv"]] %>% pull(Key) %>% first()
  }else{
    key <- storm_key
  ...
  if(!is.null(advisory_num)){
    ...
    if(sum(advisory_num %in% gis_advisories_vec) == 0){
  ...
  if(adv_num_inv | is.null(advisory_num)){
    if(download_all_advisories){
      ...
      for(i in 1:length(gis_advisories_vec)){
        invisible(cat("\tGetting data for advisory ", gis_advisories_vec[i],"\n"))
        gis_advisories_list[[i]] <- gis_advisory(key = key, advisory = gis_advisories_vec[i]) %>%
          gis_download()
      }
    }else{
      ...
      gis_advisories_list <- gis_advisory(key = key, advisory = gis_advisories_vec[length(gis_advisories_vec)]) %>%
        gis_download()
    }
  }
  return(gis_advisories_list)
}
#========================================================================================#
#=========================== End: download_storm_gis_advisories =========================#
#========================================================================================#
...
#====================================== subset_sf_obj ===================================#
#========================================================================================#
subset_sf_obj <- function(sf_obj_1 = NULL
                          , sf_obj_2 = NULL){
  # function to subset an sf object using another sf object
  # this function will give two outputs, the first one is a simple subset of 
  # the first object on the second object, in spatial data this could mean that 
  ...
  if(class(sf_obj_1) != "sf"){
    ...
    if(class(sf_obj_1) == "SpatialPolygonsDataFrame"){
      invisible(cat("\tsf_obj_1 object given is a SpatialPolygonsDataFrame object, converting it into an sf object...\n"))
      sf_obj_1 <- st_as_sf(sf_obj_1)
    }else{
      invisible(stop("\tsf_obj_1 is not a spatial object. Quitting..."))
    }
  } 
  
  if(class(sf_obj_2) != "sf"){
    invisible(cat("\tsf_obj_2 object has to be a spatial object, checking if I can convert it into one...\n"))
    if(class(sf_obj_2) == "SpatialPolygonsDataFrame"){
      invisible(cat("\tsf_obj_2 object given is a SpatialPolygonsDataFrame object, converting it into an sf object...\n"))
      sf_obj_2 <- st_as_sf(sf_obj_2)
    }else{
      invisible(stop("\tsf_obj_2 is not a spatial object. Quitting..."))
    }
  } 
  
  # make sure the two objects have the same crs 
  if(st_crs(sf_obj_1) != st_crs(sf_obj_2)){
  ...
  # sf_obj_1_subset contains any area that touches the sf_obj_2
  
  # to be more accurate, we can get only the areas in the first object that are totally 
  # within the second object
  
  # we need to get the indexes of areas in the first object that are totally within the second object
  sf_obj_1_within <- st_within(sf_obj_1, sf_obj_2)
  
  # filter the new sf_obj_1_within object
  sf_obj_1_within_2 <- map_lgl(sf_obj_1_within, function(x) {
    if (length(x) == 1) {
      return(TRUE)
    } else {
      return(FALSE)
...
get_affected_areas <- function(storm_polygon_sf = NULL
  ...
  returned_list <- list(counties_affected = counties_affected
                        , counties_within = counties_within
                        , states_affected = states_affected
                        , states_within = states_within
                        , msa_affected = msa_affected
                        , msa_within = msa_within
                        , economic_areas_affected = economic_areas_affected
                        , economic_areas_within = economic_areas_within)
